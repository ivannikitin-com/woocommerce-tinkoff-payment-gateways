<?php
/**
 * Tinkoff Payment Gateways for WooCommerce - Input Fields Class
 *
 * @version 1.6.1
 * @since   1.3.0
...
if ( ! class_exists( 'Alg_WC_Tinkoff_Payment_Gateways_Input_Fields' ) ) :

	/**
	 * Input Fields Class.
	 */
	class Alg_WC_Tinkoff_Payment_Gateways_Input_Fields {
					...
					foreach ( $_POST['alg_wc_cpg_input_fields_required'][ $data['payment_method'] ] as $required_field_name => $is_required ) {
						...
						'' === $_POST['alg_wc_cpg_input_fields'][ $data['payment_method'] ][ $required_field_name ]
							...
							$errors->add(
								...
								sprintf( __( '%s is a required field.', 'woocommerce-tinkoff-payment-gateways' ), '<strong>' . $required_field_name . '</strong>' )
							);
						}
					}
				}
		...
		public function add_input_fields_meta_box() {
			...
			if ( ! empty( $input_fields_meta ) ) {
				add_meta_box(
					...
					__( 'Payment gateway input fields', 'woocommerce-tinkoff-payment-gateways' ),
		...
		public function add_input_fields_to_order_meta( $order_id, $posted ) {
			if ( ! empty( $_POST['payment_method'] ) && isset( $_POST['alg_wc_cpg_input_fields'][ $_POST['payment_method'] ] ) ) {
				...
				if ( 'yes' === get_option( 'alg_wc_cpg_input_fields_add_order_note', 'no' ) ) {
					$note   = array();
					$note[] = __( 'Payment gateway input fields', 'woocommerce-tinkoff-payment-gateways' ) . ':';
...
return new Alg_WC_Tinkoff_Payment_Gateways_Input_Fields();
