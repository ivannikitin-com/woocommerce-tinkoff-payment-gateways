<?php
/**
 * Tinkoff Payment Gateways for WooCommerce - Gateways Class
 *
 * @version 1.6.3
 * @since   1.0.0
...
add_action( 'plugins_loaded', 'init_wc_gateway_alg_tinkoff_class' );

if ( ! function_exists( 'init_wc_gateway_alg_tinkoff_class' ) ) {

	/**
	 * Load the class for creating custom gateway once plugins are loaded.
	 */
	function init_wc_gateway_alg_tinkoff_class() {
		...
		if ( class_exists( 'WC_Payment_Gateway' ) && ! class_exists( 'WC_Gateway_Alg_Custom_Template' ) ) {
			...
			class WC_Gateway_Alg_Custom_Template extends WC_Payment_Gateway {
				...
				public function init_form_fields() {
					...
					// Form fields.
					$this->form_fields = require 'settings/class-wc-gateway-alg-tinkoff-form-fields.php';
				}
				...
				public function is_available() {
					...
					if ($this->min_amount > 0) {
						if (WC()->cart->total < $this->min_amount) {
							return false;
						}
					}

					// Check min amount.
					$min_amount = apply_filters( 'alg_wc_tinkoff_payment_gateways_values', 0, 'min_amount', $this );
				...
				public function process_payment( $order_id ) {
					...
					$statuses = alg_wc_tinkoff_payment_gateways_get_order_statuses();
					...
					return array(
						...
						'redirect' => ( '' == $this->custom_return_url ) ?
							...
							apply_filters(
								'alg_wc_tinkoff_payment_gateway_custom_return_url',
								str_replace(
				...
				public function init( $id_count ) {
					$this->id                 = 'alg_tinkoff_gateway_' . $id_count;
					$this->has_fields         = false;
					$this->method_title       = get_option( 'alg_wc_tinkoff_payment_gateways_admin_title_' . $id_count, __( 'Custom Gateway', 'woocommerce-tinkoff-payment-gateways' ) . ' #' . $id_count );
					$this->method_description = __( 'Tinkoff Payment Gateway', 'woocommerce-tinkoff-payment-gateways' ) . ' #' . $id_count;
					$this->id_count           = $id_count;
					// Load the settings.
					$this->init_form_fields();
					$this->init_settings();
					// Define user set variables.
					$this->promocode			  = $this->get_option( 'promocode', 'default' );
					...
					if (is_admin()) {
						...
						foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
							if ($this->promocode == 'default') {
							//Для кредита все позиции собираем в одну
								$names .= $cart_item['data']->get_name().', '.$cart_item['data']->get_price().' '.get_woocommerce_currency_symbol().', '.$cart_item['quantity'].' шт., ';
								//$prices += $cart_item['data']->get_price();							
							} else {
							//Для рассрочки все позиции указываем отдельно
							$products_str .= "{name: '".$cart_item['data']->get_name()."', 
								price: ".$cart_item['data']->get_price().", 
								quantity: ".$cart_item['quantity']."}, ";								
							}
						}
						//Стоимость корзины после вычета скидки
						$total_sum = WC()->cart->cart_contents_total; 
						$result_sum = $items;
						// Стоимость корзины без учета скидки
						$items = WC()->cart->subtotal;
						/*Скидки только для тех, кто берет кредит
						 Так как виджет Тинькофф контролирует совпадение суммы товаров, указанных в price и общей суммы к оплате, и не позволяет указыват отрицательные суммы,
						приходится для кредита все товары указывать в одной позиции одной общей суммой*/
						if (WC()->cart->shipping_total > 0) {
							$items = $items + WC()->cart->shipping_total;
						}						
						if ($this->promocode == 'default') {

							$discount = $total_sum - $items;
							$names .= 'Скидка: '.$discount." ".get_woocommerce_currency_symbol();
							$products_str .= "{name: '".$names."', price: ".$total_sum.", quantity: 1}, ";
						} else {		
							$total_sum = WC()->cart->subtotal;
						}
						if (WC()->cart->shipping_total > 0) {
							//Итоговая сумма к оплате
							$result_sum = $total_sum + WC()->cart->shipping_total; 							
							$products_str .= "{name: 'Доставка', price: ".WC()->cart->shipping_total.", quantity: 1}"; 
						}
						$products_str .= ']';
/*
...
*/
						$title = '<button
							  type="button" id="alg_tinkoff_gateway_' . $id_count.'" 
							  class="TINKOFF_BTN_YELLOW TINKOFF_SIZE_L"
							  >'.$this->method_title.'</button>
							    <script>
							    document.getElementById("alg_tinkoff_gateway_' . $id_count.'").onclick = function(event) {
									    ...
									    let billingPhone = document.getElementById("billing_phone").value;
									    let billingEmail = document.getElementById("billing_email").value;

									    if ((billingLastName !== "") && (billingFirstName !== "")) {
									    	params.values = {
									    		contact: {
									    			fio: {
...
/*							    tinkoff.createDemo(
				...
				public function tinkoff_button_attributes($title, $id) {
					if (is_checkout() && $id=='alg_tinkoff_gateway_1') {
						$products = "[{name: 'iphone 11', price: 100000, quantity: 1}, {name: 'Чехол', price: 500, quantity: 1}]";
						$title = '<button
							  type="button"
							  class="TINKOFF_BTN_YELLOW TINKOFF_SIZE_L"
							  onclick="tinkoff.createDemo(
							...
							>'.__('Купить в кредит','woocommerce-tinkoff-payment-gateways').'</button>';
					}
					if (is_checkout() && $id=='alg_tinkoff_gateway_2') {
						$products = "[{name: 'iphone 11', price: 100000, quantity: 1}, {name: 'Чехол', price: 500, quantity: 1}]";
						$title = '<button
							  type="button"
							  class="TINKOFF_BTN_YELLOW TINKOFF_SIZE_L"
							  onclick="tinkoff.createDemo(
							    {'."
							      ...
							      items: ".$products.",
							      demoFlow: 'sms',
							      promoCode: '".$this->get_option( 'promocode', 'default' )."',
							      shopId: 'SHOP_ID',
							      showcaseId: 'SHOWCASE_ID',
							    },
							    {view: 'modal'}".'
							  )"
							>'.__('Купить в рассрочку','woocommerce-tinkoff-payment-gateways').'</button>';
					}			
					return $title;
				}

				public function wc_checkout_discount_amount(){
					$total_regular=0;
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			 ...
			 * @since   1.0.0
			 */
			function add_wc_gateway_alg_tinkoff_classes( $methods ) {
				$_methods = array();
				for ( $i = 1; $i <= apply_filters( 'alg_wc_tinkoff_payment_gateways_values', 1, 'total_gateways' ); $i++ ) { //phpcs:ignore
			...
			add_filter( 'woocommerce_payment_gateways', 'add_wc_gateway_alg_tinkoff_classes' );
